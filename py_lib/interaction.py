...
BREAKPOINT_REFACTORING_TOOL. Он инкапсулирует в себе всю логику двустороннего 
консольного диалога с оператором, пошагового контроля вычислений, ручной валидации 
данных и построчного ввода кастомных инженерных переводов.

Архитектура модуля разделена на 3 функциональных слоя:
1. ЦЕНТРАЛИЗОВАННЫЙ ПЕРЕХВАТ ПРЕРЫВАНИЙ (Signal & Stream Handling)
   Служебные компоненты нижнего уровня для защиты потока ввода от аварийных сигналов 
   терминала (Ctrl+C, EOFError). Изолируют диалоги подтверждения выхода, исключая 
   дублирование громоздких блоков try-except в коде верхних уровней.
2. УНИВЕРСАЛЬНЫЕ КОМПОНЕНТЫ УПРАВЛЕНИЯ ШАГАМИ (Pipeline Control UI)
   Инструменты оркестровки шагов обработки (паузы, очистка экранов, рендеринг 
   заголовков) и управления снимками состояний (rollback/retry) в оперативной памяти 
   для обеспечения возможности безопасного отката изменений.
3. БИЗНЕС-СПЕЦИФИЧНЫЕ ФОРМЫ ВВОДА И ВАЛИДАЦИИ (Data Ingestion Forms)
   Высокоуровневые интерактивные интерфейсы для ручного распределения статусов 
   документов, массового пакетного заполнения атрибутов технологических изменений 
   (BP) и матричного вывода результатов (Data Preview) с автоподгонкой ширины ячеек.

Правила взаимодействия с бизнес-логикой:
    - Модуль является "чистым" интерфейсным представлением и не производит 
      инженерных расчетов, нормализации или физической записи файлов на диск.
    - Вся собранная пользовательская информация возвращается наверх в виде 
      базовых структур данных (str, dict, bool) или модифицированных копий DataFrame.

Версия: 1.0
Совместимость: Python 3.14.4, Pandas 3.0.3
Поддержка: PLD Engineering Center
Дата изменения: 2026-09-10
Лицензия: MIT
Статус: Production
"""
import sys
